...
gulp.task('build:app:assets', function (){
    gulp.src('src/app/**/*.?(png|jpg|gif|js)', option)
        .pipe(gulp.dest(dist))
        .pipe(browserSync.reload({stream: true}));
});

gulp.task('build:app:style', function (){
    gulp.src('src/app/app.less', option)
        .pipe(less().on('error', function (e){
            console.error(e.message);
            this.emit('end');
        }))
...
gulp.task('build:app:html', function (){
    gulp.src('src/app/index.html', option)
...
gulp.task('build:app', ['build:app:assets', 'build:app:style', 'build:app:html']);

gulp.task('release', ['build:style', 'build:app']);

gulp.task('watch', ['release'], function () {
    gulp.watch('src/style/**/*', ['build:style']);
    gulp.watch('src/app/app.less', ['build:app:style']);
    gulp.watch('src/app/**/*.?(png|jpg|gif|js)', ['build:app:assets']);
